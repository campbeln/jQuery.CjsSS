/*
$.CjsSS v0.9k (kk) http://opensourcetaekwondo.com/$cjsss/
...
(function (jQuery, _window, _document, fnCjs3, fnEvalerFactory, fnLocalEvaler, fnUseStrictEvaler, fnSandboxEvalerFactory) {
    var fnImplementation = function ($services) {   //# $.CjsSS functionality
            'use strict';
            ...
            var cjsss = "cjsss",
                ...
                $cjsss = {
                    ...
                    options: {                      //# STRING (CSS Selector); See: http://lesscss.org/#client-side-usage , http://stackoverflow.com/questions/7731702/is-it-possible-to-inline-less-stylesheets
                        ...
                        optionScope: "json",        //# STRING/Enum; Method to evaluate option code within.
                        scope: "isolated",          //# STRING/Enum; Javascript scope to evaluate code within.
                    ...
                    process: function () {
                        $services.jq.process.apply(this, arguments);
                    },
                    mixin: function () {
                        $services.mixin.apply(this, arguments);
                    },
                    inject: function () {
                        $services.jq.inject.apply(this, arguments);
                    }
                },
                oDefaultOptions = $cjsss.options    //# Include these alias(es) variables for improved code minimization
            ;
    
            //# Set our .version and .extend the passed $services with our own Vanilla Javascript (js) logic then .run
            ...
            $services.jq = $services.extend(
                fnVanillaJS($services, oDefaultOptions, $cjsss.data.inject),
                {
                    //# Processes the elements referenced by the passed oProcessOptions (defaulting to the oDefaultOptions.selector)
                    process: function(oProcessOptions) {
                        var vSelector = ($services.is.obj(oProcessOptions) ? oProcessOptions.selector : null) || oDefaultOptions.selector;
                        jQuery(vSelector)[cjsss](oProcessOptions);
                    },
                    
                    
                    //# Runs the fnImplementation logic while configuring the base $services object
                    run: function () {
                        var oBase = {},
                            oExtension = {}
                        ;
                        ...
                        //# .inject our .mixin
                        $services.jq.inject("mixin", $services.mixin);
                        
                        //# Setup the jQuery $('selector').cjsss() oExtension
                        oExtension[cjsss] = {
                            cjsss: function (oProcessOptions) {
                                var oPrelimOptions = $services.extend({}, oDefaultOptions, oProcessOptions);
                        
                                //# Traverse the jQuery selected elements, .compiler'ing .each as we go
                                return this.each(function () {
                                    $services.jq.compiler(this[0], oPrelimOptions);
                                });
                            }
                        };
                        jQuery.fn.extend(oExtension);
                    
                        //# Create the jQuery.cjsss oBase interface
                        oBase[cjsss] = $cjsss;
                        jQuery.extend(oBase);
                    } //# run                    
                }
        ...
        fnVanillaJS = function ($services, oDefaultOptions, oInjections) {      //# Define the VanillaJS $services
            var reScript = /<script.*?>([\s\S]*?)<\/script>/gi,
            
                //# .extend the passed $services with our the fnVanillaJS (.js) logic
                //#      NOTE: Setting up $services like this allows for any internal logic to be overridden as well as allows for all versions of CjsSS to coexist under a single definition (though some .conf logic would need to be used)
                $VJSServices = {
                    ...
                    //# Compiles the CSS within the passed vElements using the provided oProcessOptions (overriding any previously set)
                    //#     TODO: May need to refactor out all of the pre-traversal stuff into .js definition above (as it's all non-jQuery logic anyway), move "process" logic to above and make this "compile"?
                    compiler: function (a__ElementsToCompile, oPrelimOptions) {
                        var i,
                            a__CompiledElements = []
                        ;
                            ...
                            a__CompiledElements = a__CompiledElements.concat(
                                $services.compile.element(
                                    ...
                                    function (oCacheEntry /*, a__Elements*/) {
                                        ...
                                        oCacheEntry.$interpolate = $VJSServices.interpolate(oCacheEntry, oPrelimOptions);
